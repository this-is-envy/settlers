...
static func FromQR(q_in: int, r_in: int) -> TileCoords:
  var ac := TileCoords.new()
  ac.q = q_in
  ac.r = r_in
...
static func FromXY(x: int, y: int) -> TileCoords:
  var ac := TileCoords.new()
...
static func FromXYVector(xy: Vector2) -> TileCoords:
